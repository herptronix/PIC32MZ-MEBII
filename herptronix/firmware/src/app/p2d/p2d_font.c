...
static void PutGlyph(const rect_st *rec, const uint8_t *ptrGlyph) {
  ...
  if(cntPxClip == cntPxGly && fontType == FONT_4BPP) {
    ...
    if(context.mode == DISPLAY_SOLID) {
      if(context.alpha == 255) {
        for(y = rec->y; y < rec->y + rec->h; y++) {
          p = &pDest[(uint32_t)GetWidth() * y + rec->x];
          pEnd = p + rec->w;
          while(p < pEnd) {
            color = *ptrGlyph & mask;
        ...
        for(y = rec->y; y < rec->y + rec->h; y++) {
          ...
          while(p < pEnd) {
            color = *ptrGlyph & mask;
            if(mask == 0xF0) color >>= 4;
            *p = alphablend(lut.lut[color], *p, context.alpha);
            p++;
            NextBit_4BPP(&mask, &ptrGlyph);
          }
        }
      ...
      for(y = rec->y; y < rec->y + rec->h; y++) {
        ...
        while(p < pEnd) {
          alpha = *ptrGlyph & mask;
          if(mask == 0xF0) alpha >>= 4;
          if(alpha) {
            *p = alphablend(context.colFront, *p, ((uint16_t)context.alpha * alpha) / 0xF);
          }
          p++;
          NextBit_4BPP(&mask, &ptrGlyph);
        }
  ...
  //TODO: complete for all cases
    ...
    if(fontType == FONT_4BPP) PutSlow_4BPP(rec, ptrGlyph);
  }
}
